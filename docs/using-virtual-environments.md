...
### __Note__
- The instructions are applied to a Windows machine only, since we're setting up for 3D Pinball which doesn't work on other platforms.
...
1. Verify pip version is the same as in the __Install Pip__ section. 
    - If you don't have the latest pip in the virtual environment, you can upgrade it by executing `python -m pip install --upgrade pip`
